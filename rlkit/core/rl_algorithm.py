...
class RLAlgorithm(metaclass=abc.ABCMeta):
    ...
    def train_online(self, start_epoch=0):
        ...
        for epoch in gt.timed_for(
            ...
            for _ in tqdm(range(self.num_env_steps_per_epoch)):
                # 执行一步模拟操作，并返回下一步的状态，将这一次样本加入到Replay_buffer中
    ...
    def _try_to_eval(self, epoch, eval_paths=None):
        ...
        if self._can_evaluate():
            ...
            logger.record_tabular("Train Time", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
            logger.record_tabular("Patient Name", self.env.patient_name)
