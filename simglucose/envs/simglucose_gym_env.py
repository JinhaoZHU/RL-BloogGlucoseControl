...
class DeepSACT1DEnv(gym.Env):
    ...
    def _step(self, action, cho=None, use_action_scale=True):
        """
        进行一步模拟，也就是对虚拟患者注射胰岛素和饮食等外部动作，然后患者状态进入下一个时刻
        :param action:
        :param cho:
        :param use_action_scale:
        :return:
        """
    ...
    def calculate_iob(self):
        # 猜测iob是胰岛素的衰减速率，也就是前一段时间胰岛素在当前时刻的残留率
        # np.dot为点积，根据残留率，计算当前所有的胰岛素量
    ...
    def get_state(self, normalize=False):
        ...
        # 基础的返回值是两个，bg和insulin
        return_arr = [bg, insulin]

        # 关于时间的信息，加入过去一段时间具体在一天内的分钟数
        if self.time:
        ...
        # 关于饮食的信息，将前一段时间的饮食信息加入到状态中
        if self.meal:   # 如果为True，就将饮食信息加入到
            # cho = self.env.CHO_hist[-self.state_hist:]
            cho = self.env.CHO_hist[-int((2 * 60) / self.sample_time):]
            if normalize:
                cho = np.array(cho)/20.
        ...
        #
        if self.fake_real:
            # env.patient：T1DPatientNew
            # state为vpatient_params.csv中的前13个参数
    ...
    def set_patient_dependent_values(self, patient_name):
        ...
        self.ideal_basal = self.bw * self.u2ss / 6000.      # 最佳胰岛素基础率
        ...
        self.iob = iob_all[self.patient_name] # 长度为65的数组
